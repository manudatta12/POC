...
(function($) {
	var conf = {},
		...
		numMSG = 15; // set the number of messages to be show
		...
		live:"indvswi", //optional, disabled by default, the number after "live-" indicates the time in seconds to wait before request the Twitter API for updates, I do not recommend to use less than 60 seconds.
					...
					$.each(json.results,function(i,item) {
						if(e==1 || (i<numMSG && item.id>ultID)){
							...
							if (item.text != "undefined") {
								...
								if(fromID=="image") mHTML="<div style='clear:both;  padding:10px' class='txtdellnrml'><a href='http://www.twitter.com/"+item.from_user+"'><img src='"+item.profile_image_url+"' alt='"+item.from_user+"' class='juitterAvatar' align='left' style='padding-right:10px' /></a> "+$.Juitter.textFormat(tweet)+" <div class='linkatweet'><a href='" + link + openLink+" >"+'// '+readMore+"</a></div></div>";
